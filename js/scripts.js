        ...
        try {
            this.road.obstacles.forEach(obstacle => {
                ...
                if (obstacle.calculateHit(this.car)) {
                    this.gameOver();
                    throw new DOMException(); // random exception, does not really matter here
                }
            });
        } catch (e) {
            //do nothing, just to break from forEach lambda
        }
...
class Road {
    ...
    //todo: could be done with one generate method
        ...
        truck.lineCords.forEach(cord => {
            if (counter % 2 === 1) {
                ...
                if (counter === 5 && verticalRand > 0) {
                    verticalRand = -verticalRand; // so they dont miraculously appear on already visible route
                }
    ...
    intersects(firstX1, firstY1, firstX2, firstY2, secondX1, secondY1, secondX2, secondY2) {
        ...
        if (det === 0) {
            ...
            // minimal error is also valid as intersection
        ...
        trucks.forEach(truck => {
            for (let i = 0; i < truck.lineCords.length - 1; i++) {
                if (truck.lineCords[i + 1].y >= y &&
                    ...
                    //this complicated code is for detection of roadside collision
                    let leftLine = {
                        start: {
                            x: truck.lineCords[i].x - (Road.ROAD_WIDTH + 50) / 2,
                            y: truck.lineCords[i].y - 30
                        },
                        end: {
                            x: truck.lineCords[i + 1].x - (Road.ROAD_WIDTH + 50) / 2,
                            y: truck.lineCords[i + 1].y - 30
                        }
                    };
                    let rightLine = {
                        start: {
                            x: truck.lineCords[i].x + (Road.ROAD_WIDTH + 100) / 2,
                            y: truck.lineCords[i].y - 30
                        },
                        end: {
                            x: truck.lineCords[i + 1].x + (Road.ROAD_WIDTH + 100) / 2,
                            y: truck.lineCords[i + 1].y - 30
                        }
                    };
                    let leftLine2 = {
...
class RoadPreset {
    ...
    constructor(ctx) {
        ...
        // we keep begin and end cords constant to be able to easily "glue" the road
        // it does not worsen the game experience in any way as it's hardly detectable
    ...
    draw(Y) {
        //todo: some code from here could be moved to separate methods
        this.ctx.translate(0, 0);
        this.ctx.save();
        this.ctx.beginPath();
        ...
        // we update the cords because other object will use those
